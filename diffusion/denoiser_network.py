...
class ResidualMLPDenoiser(nn.Module):
    def __init__(
        ...
        self.residual_mlp = ResidualMLP(
            ...
            output_dim=d_in,
            activation=cfg.diff_activation,
            layer_norm=cfg.layer_norm,
        )
        if cond_dim is not None:
            self.proj = nn.Linear(d_in + cond_dim, cfg.dim_t)
            self.conditional = True
        else:
            self.proj = nn.Linear(d_in, cfg.dim_t)
            self.conditional = False

        # time embeddings
        self.random_or_learned_sinusoidal_cond = cfg.learned_sinusoidal_cond or cfg.random_fourier_features
        if self.random_or_learned_sinusoidal_cond:
            sinu_pos_emb = RandomOrLearnedSinusoidalPosEmb(cfg.learned_sinusoidal_dim, cfg.random_fourier_features)
            fourier_dim = cfg.learned_sinusoidal_dim + 1
        else:
            sinu_pos_emb = SinusoidalPosEmb(cfg.dim_t)
            fourier_dim = cfg.dim_t

        self.time_mlp = nn.Sequential(
            sinu_pos_emb,
            nn.Linear(fourier_dim, cfg.dim_t),
            nn.SiLU(),
            nn.Linear(cfg.dim_t, cfg.dim_t)
        )
