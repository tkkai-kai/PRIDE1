...
for spec in ${GROUP_SPECS}; do
    ...
    for seed in ${SEED_LIST}; do
        ...
        export_vars="ALL,PRIDE_CLUSTER=${PRIDE_CLUSTER},GRP_LABEL=${grp_label},DIFFUSION_SAMPLE_RATIO=${DIFFUSION_SAMPLE_RATIO},USE_SYNTHETIC_REWARD_DATA=false,SYNTHETIC_REWARD_RATIO=0.0,MAX_FEEDBACK=${MAX_FEEDBACK},RETRAIN_DIFFUSION_EVERY=${RETRAIN_DIFFUSION_EVERY},TRAIN_BATCH_SIZE=${TRAIN_BATCH_SIZE},NUM_TRAIN_STEPS=${NUM_TRAIN_STEPS},DIFFUSION_WARM_START=${DIFFUSION_WARM_START},USE_UNCERTAINTY_FILTER=true,UNCERTAINTY_KEEP_QUANTILE=${keep_q},SEED_LIST=${seed},FEED_TYPE=${FEED_TYPE},RUN_DATE=${RUN_DATE},OUTPUT_NAME=${output_name}"
