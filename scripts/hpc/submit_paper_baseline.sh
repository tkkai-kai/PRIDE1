...
echo "oracle=true feed_type=0 segment=50 unsupervised_steps=9000"
...
for seed in "${SEEDS[@]}"; do
    ...
    if [[ "${DRY_RUN}" == "1" ]]; then
        printf 'DRY_RUN: sbatch --partition=%q --qos=%q --gres=%q --time=%q --exclude=%q --job-name=%q --output=%q --error=%q --export=%q %q\n' \
            "${SLURM_PARTITION}" "${SLURM_QOS}" "${SLURM_GRES}" "${SLURM_TIME}" \
            "${SLURM_EXCLUDE}" "${job_name}" "${stdout}" "${stderr}" "${export_vars}" "${JOB_BODY}"
    else
        pride_sbatch_submit \
            --job-name="${job_name}" \
            --output="${stdout}" \
