...
class DiffReplayBuffer(ReplayBuffer):
    """Extended version of ReplayBuffer to handle differential updates."""
    def __init__(self, obs_shape, action_shape, capacity, device, window=1):
    ...
    def sample_batch(self, batch_size=32, idxs=None):
        ...
        return dict(obs1=self.obses[idxs],
                    obs2=self.next_obses[idxs],
                    acts=self.actions[idxs],
                    rews=self.rewards[idxs],
                    done=1 - self.not_dones_no_max[idxs],
                    idxs=idxs)
