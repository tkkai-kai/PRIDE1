...
class SACAgent(Agent):
    ...
    def combine_data(self, replay_buffer, diff_replay_buffer, online_batch_size, diffusion_batch_size):
        obs, action, reward, next_obs, not_done, not_done_no_max = replay_buffer.sample(diffusion_batch_size)
        diff_obs, diff_action, diff_reward, diff_next_obs, diff_not_done, diff_not_done_no_max = diff_replay_buffer.sample(online_batch_size)

        obs_tensor = combine_two_tensors(obs, diff_obs).to(self.device)
        obs_next_tensor = combine_two_tensors(next_obs, diff_next_obs).to(self.device)
        acts_tensor = combine_two_tensors(action, diff_action).to(self.device)
        rews_tensor = combine_two_tensors(reward, diff_reward).to(self.device)
        not_done_tensor = combine_two_tensors(not_done, diff_not_done).to(self.device)
        not_done_no_max_tensor = combine_two_tensors(not_done_no_max, diff_not_done_no_max).to(self.device)
        return obs_tensor, acts_tensor, rews_tensor, obs_next_tensor, not_done_tensor, not_done_no_max_tensor


    def update(self, replay_buffer, logger, step, gradient_update=1, diff=False, diff_replay_buffer=None, ratio=0):
        for index in range(gradient_update):
            if diff:
                diffusion_batch_size = int(self.batch_size * ratio)
                online_batch_size = int(self.batch_size - diffusion_batch_size)
                if diff_replay_buffer.size < diffusion_batch_size:
                    obs, action, reward, next_obs, not_done, not_done_no_max = replay_buffer.sample(
                        self.batch_size)
                else:
                    obs, action, reward, next_obs, not_done, not_done_no_max = self.combine_data(
                        replay_buffer, diff_replay_buffer, online_batch_size, diffusion_batch_size)
            else:
                obs, action, reward, next_obs, not_done, not_done_no_max = replay_buffer.sample(
                    self.batch_size)
